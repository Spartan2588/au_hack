...
export class ScenarioChat {
  ...
  async fetchCurrentMetrics() {
    ...
    try {
      ...
      const cityId = this.selectedCity.toLowerCase() === 'mumbai' ? 1 : 
                     this.selectedCity.toLowerCase() === 'delhi' ? 2 : 
                     this.selectedCity.toLowerCase() === 'bangalore' ? 3 : 1;
      
      console.log(`[ScenarioChat] Using cityId: ${cityId}`);
      
      const data = await this.api.getCurrentState(cityId);
      console.log(`[ScenarioChat] Received data:`, data);

      // Store metrics
      this.currentMetrics = {
        aqi: data.aqi || 100,
        temperature: data.temperature || 30,
        hospital_load: data.hospital_load || data.bed_occupancy_percent || 50,
        crop_supply: data.crop_supply || data.crop_supply_index || 70,
        timestamp: data.timestamp,
        data_freshness: data.data_freshness || 'recent'
      };

      console.log(`[ScenarioChat] Processed metrics:`, this.currentMetrics);
      ...
      <div class="current-metrics-card">
        ...
        <div class="metrics-row">
          ...
          <div class="metric-item">
            ...
            <span class="metric-value">${(data.crop_supply || data.crop_supply_index)?.toFixed(0) || 'N/A'}%</span>
          </div>
        </div>
      </div>
    `;
